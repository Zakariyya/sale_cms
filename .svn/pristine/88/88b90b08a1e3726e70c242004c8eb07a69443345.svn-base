/**
 * 因为一个html只能执行一个ready方法。
 * 故将所有需要在ready时进行的方法全都
 */
$(document).ready(function(){
	...
	$("#modifySavePassword").click(function(){
		...
		if("" == oldpassword){	//原密码为空
			tip.css("color","red");
			tip.html("对不起，请输入原密码，谢谢。");
			$("#oldloginpwd").focus();
		}else if("" == newpassword){	//新密码为空
			tip.css("color","red");
			tip.html("对不起，请输入新密码，谢谢。");
			$("#newpassword").focus();
		}else if(newpassword.length < 6){	//新密码长度小于6
			tip.css("color","red");
			tip.html("对不起，密码长度不得小于6位，请重试。");
			$("#newpassword").focus();
		}else if("" == aginpassword){	//第二新密码为空
			tip.css("color","red");
			tip.html("对不起，请再次输入新密码，谢谢。");
			$("#aginpassword").focus();
		}else if(newpassword != aginpassword){	//两次输入不符
			tip.css("color","red");
			tip.html("对不起，两次密码输入不符合，重新输入，谢谢。");
			$("#aginpassword").val("");
			$("#aginpassword").focus();
		}
		else{
			...
			$.ajax({
				...
				dataType:'html', //ajax返回的数据类型
				timeout:1000,
				error:function(){
					alert("error: 修改密码失败，请重试。");
				},
				success:function(result){
					if(result != "" && "success" == result){
						tip.css("color","green");
						tip.html("修改密码成功，下次登录记得使用新密码");
					}else if("filed" == result){
						tip.css("color","red");
						tip.html("修改密码失败，请重试");
					}else if("oldpasswrong" == result){
						tip.css("color","red");
						tip.html("原密码不正确，请重试");
					}else if("nodata" == result){
						tip.css("color","red");
						tip.html("无数据需要操作，请重试");
					}
				}
			});
		}
	...
	for(var i = 0;i< json.length;i++){
		...
		for(var j = 0;j < json[i].subMenu.length;j++){
			...
			result = result + "<li><a class=\"ajax-link\" style=\"cursor:pointer;\" href=\""+json[i].subMenu[j].funcUrl +"\"><i class="+pic+"></i><span class=\"hidden-tablet\">"+json[i].subMenu[j].functionName + "</span></a></li>";
		}
		result = result+"</ul></li>";
	}
